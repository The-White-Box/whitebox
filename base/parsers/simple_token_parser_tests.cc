...
GTEST_TEST(SimpleTokenParserTest, ParseToken) {
  ...
  constexpr std::array<std::tuple<std::string_view, ParsedToken>, 62> cases{{
      ...
      std::tuple{" ", ParsedToken{}},
      std::tuple{"  ", ParsedToken{}},
      std::tuple{" a", ParsedToken{.next_token = "", .current_token = "a"}},
      ...
      std::tuple{" /", ParsedToken{.next_token = "", .current_token = "/"}},
      std::tuple{" /a", ParsedToken{.next_token = "", .current_token = "/a"}},
      std::tuple{" /ac", ParsedToken{.next_token = "", .current_token = "/ac"}},
      std::tuple{" /de/",
                 ParsedToken{.next_token = "", .current_token = "/de/"}},
      // Invalid case.
      std::tuple{" /fg/\"",
                 ParsedToken{.next_token = "", .current_token = "/fg/\""}},
      std::tuple{" /eh/ \"",
      ...
      // Invalid case.
      std::tuple{"a\"b",
                 ParsedToken{.next_token = "", .current_token = "a\"b"}},
      std::tuple{"a \"c",
